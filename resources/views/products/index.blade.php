...
@media (max-width: 768px) {
    ...
    .slider {
            height: 60vh; /* Уменьшаем высоту слайдера на мобильных */
        } 
...
@media (max-width: 480px) {
    .slider {
        height: 50vh; /* Еще меньше на очень маленьких экранах */
    }

    .caption {
        font-size: 14px; /* Еще меньше на очень маленьких экранах */
        bottom: 5px; /* Поднимаем текст немного выше */
    }

    .slider .prev,
    .slider .next {
        height: 25px; /* Уменьшаем высоту кнопок для iPhone SE */
        width: 25px; /* Уменьшаем ширину кнопок для iPhone SE */
        left: 5px; /* Сдвигаем кнопки ближе к краю */
        right: 5px; /* Сдвигаем кнопки ближе к краю */
    }
    ...
    .slider .next {
        top: 50%;
        transform: translateY(-50%) scale(0.7); /* Уменьшаем масштаб кнопок */
    }
}
</style>
...
@section('search')
    <h1 style="font-family: 'Tenor Sans', sans-serif; font-size: 60px; color: #005154; padding-left: 100px; padding-top: 80px" class="h1">КУПИТЬ ЯХТУ</h1>
...
@section('feedback')
    ...
    <form id="contact-form" action="{{ route('contact.store') }}" method="POST">
        ...
        <div class="feedback-text2">Оставьте заявку, и мы сделаем для вас индивидуальную подборку яхт. Нам всегда есть, что предложить.</div>
...
<style>
    ...
    margin-top: 50px;
    width: 100%;
    padding: 0 100px; /* Убираем высоту, чтобы она адаптировалась к содержимому */
...
.input-container {
    position: relative;
    margin: 20px 0;
    flex: 1; /* Делает все input-container одинаковой ширины */
    min-width: 250px; /* Минимальная ширина контейнера для инпута */
}
...
.Button__StyledButton:hover {
    ...
    border: 1px solid;
    padding: 19.5px 57px; /* Учитываем изменение размера */
}
...
@media (max-width: 768px) {
    .feedback {
        ...
        height: auto; /* Разрешаем высоте адаптироваться к содержимому */
    }
    ...
    .feedback-inputs {
        flex-direction: column; /* Смена направления на столбик */
        gap: 15px; /* Увеличиваем отступы между инпутами */
    }

    .input-container {
        width: 100%; /* Делает контейнеры инпутов более адаптивными */
        margin: 0; /* Убираем отступы по вертикали */
    }

    .input-container input {
        max-width: none; /* Ограничиваем ширину для мобильных */
        font-size: 14px; /* Уменьшаем размер шрифта в полях ввода */
    }

    .Button__StyledButton {
        font-size: 14px; /* Уменьшаем размер шрифта на кнопке */
        padding: 15px 40px; /* Уменьшаем отступы на кнопке */
    }
...
@media (max-width: 500px) {
    ...
    .feedback-text2 {
        font-size: 12px; /* Уменьшаем размер подзаголовка для маленьких экранов */
        text-align: center !important;
        padding: 0;
        margin-bottom: 10px;
    }
    ...
    .Button__StyledButton {
        padding: 15px 30px; /* Уменьшаем отступы на кнопке для маленьких экранов */
    }

    .FeedbackFormstyled {
        font-size: 10px; /* Дополнительное уменьшение размера текста в соглашении */
    }
}
</style>
...
<style>
    ...
    .info-buy-info a:hover {
        ...
        color: #000000CC; /* Цвет ссылки при наведении */
    }
    ...
    @media (max-width: 768px) {
        ...
        .info-buy h2 {
            ...
            margin-bottom: 40px; /* Уменьшаем отступ снизу заголовка */
        }
    ...
    @media (max-width: 480px) {
        .info-buy h2 {
            font-size: 24px; /* Дополнительное уменьшение размера заголовка для маленьких экранов */
            margin-bottom: 30px; /* Уменьшаем отступ снизу заголовка еще больше */
        }

        .info-buy-info {
            font-size: 12px; /* Дополнительное уменьшение размера текста для маленьких экранов */
        }
    }
</style>
@endsection
...
<style>
    ...
    @media (max-width: 768px) {
        .location {
            height: auto; /* Позволяем высоте адаптироваться к содержимому на мобильных устройствах */
        }

        .info {
            top: auto; /* Убираем вертикальное центрирование */
            bottom: 20px; /* Устанавливаем отступ снизу */
            right: 10px; /* Уменьшаем отступ справа */
            padding-right: 20px; /* Уменьшаем отступ внутри блока информации */
            text-align: left; /* Выравнивание текста по левому краю для мобильных */
            transform: none; /* Убираем трансформацию для мобильных устройств */
        }
    ...
    @media (max-width: 480px) {
        .info {
            bottom: 10px; /* Уменьшаем отступ снизу на очень маленьких экранах */
            right: 5px; /* Уменьшаем отступ справа на очень маленьких экранах */
            padding-right: 10px; /* Уменьшаем отступ внутри блока информации еще больше */
        }

        .info h2 {
            font-size: 18px; /* Дополнительное уменьшение размера заголовка для маленьких экранов */
        }

        .info p {
            font-size: 14px; /* Дополнительное уменьшение размера текста для маленьких экранов */
        }
    }
</style>
@endsection
...
<footer>
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <p style="font-family: 'Tenor Sans', sans-serif;">Компания Arcon Yachts предоставляет своим клиентам широкий спектр услуг в сфере яхтенной индустрии.</p>
            </div>
            <div class="col-md-4" style="text-align: center;">
                <h5 style="font-size: 26px; font-family: 'Golos Text', sans-serif;">Контакты</h5>
            ...
            <div class="col-md-4">
                <ul class="social-links" style="list-style-type: none; padding: 0; text-align: right;">
                    <li><h5 style="font-size: 26px; font-family: 'Golos Text', sans-serif;">Социальные сети</h5></li>
                    <li style="font-size: 18px;"><a href="https://www.instagram.com/arconyachtsmonaco/"><i class="fab fa-instagram"></i> Instagram</a></li>
                    <li style="font-size: 18px;"><a href="https://www.facebook.com/arconyachts/"><i class="fab fa-facebook-f"></i> Facebook</a></li>
                    <li style="font-size: 18px;"><a href="https://www.linkedin.com/company/arcon-yachts/"><i class="fab fa-twitter"></i> Twitter</a></li>
                </ul>
            </div>
        </div>
...
<style>
    footer {
        background-color: #005154; /* Темный фон */
        color: white; /* Белый текст */
        padding: 40px 0 20px 0; /* Отступы сверху и снизу */
    }
    ...
    .text-center {
        margin-top: 20px; /* Отступ сверху для текста в центре */
    }
    ...
    @media (max-width: 768px) {
        ...
        footer h5 {
            font-size: 22px; /* Уменьшаем размер заголовка на мобильных */
        }
        ...
        .social-links {
            align-items: center; /* Центрируем социальные ссылки */
            margin-top: 15px; /* Отступ сверху для социальных ссылок */
        }
    }
    
    @media (max-width: 480px) {
        footer h5 {
            font-size: 20px; /* Дополнительное уменьшение для очень маленьких экранов */
        }

        footer p, footer ul li {
            font-size: 14px; /* Еще меньше размер текста на маленьких экранах */
        }
        
        .social-links {
            gap: 10px; /* Уменьшаем отступ между иконками на маленьких экранах */
        }
    }
</style>
@endsection
