...
<!-- @if(isset($item['data']) && !empty($item['data']))
    <div class="data-yahts font_2">
        @if(isset($item['data']['year'])) 
            <p>{{ $item['data']['year'] }} | </p>
        @endif
        @if(isset($item['data']['length']))
            <p>{{ $item['data']['length'] }} м | </p>
        @endif
        @if(isset($item['data']['guests']))
            <p>{{ $item['data']['guests'] }} гостей | </p>
        @endif
        @if(isset($item['data']['rooms']))
            <p>{{ $item['data']['rooms'] }} кают</p>
        @endif
    </div>
@else
    <p>Информация о продукте недоступна.</p>
@endif -->
...
@endforeach
                        ...
                        @else
                            <div class="product-info">
                                <p colspan="5" align="center"><h4>Корзина пуста</h4></p>
                            </div>
                        @endif
                    </tbody>
                </table>
...
<style>
    ...
    .product-image {
        max-width: 100%; /* Увеличьте размер изображения */
        height: auto;
    }

    .product-info {
        text-align: left; /* Выравнивание текста под изображением */
        display: flex; 
